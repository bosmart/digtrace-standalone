...
def write_ply(filename, points_np=None, mesh=None, as_text=False):
    """
    Parameters
    ----------
    filename: str
    ...
    with open(filename, 'w') as ply:
        ...
        if points_np is not None:
            if points_np.shape[1]==3:
                points = pd.DataFrame(data=points_np, columns=['x', 'y', 'z'])
            elif points_np.shape[1]==6:
                points = pd.DataFrame(data=points_np, columns=['x', 'y', 'z', 'red', 'green', 'blue'])
                points['red'] = points['red'].astype(np.uint8)
                points['green'] = points['green'].astype(np.uint8)
                points['blue'] = points['blue'].astype(np.uint8)
