...
class Freezer(object):
    ...
    def __init__(self, executables, constantsModules = [], includes = [],
            ...
            includeFiles = [], zipIncludes = [], zipPackages = [],
            silent = False, namespacePackages = [], metadata = None,
            includeMSVCR = False):
        ...
        self.zipPackages = set(zipPackages)
        self.silent = silent
    ...
    def _WriteModules(self, fileName, initScript, finder, compress,
        ...
        # Copy any package data for non-zip packages to the destination
        # directory. This needs to be before we copy the modules, because
        # shutil.copytree assumes that the destination directory doesn't
        # already exist
        for package in self.packages:
            if package in self.zipPackages:
                continue
            module = finder._modules[package]
            if not os.path.isdir(module.file):
                # Modules may also be listed in packages
                continue
            destination = os.path.join(targetDir, package)
            shutil.copytree(module.file, destination,
                        ignore=shutil.ignore_patterns('*.py', '*.pyc', '*.pyo'))
        ...
        filesToCopy = []
        for module in modules:
            pkgname = module.name.split('.')[0]
            in_package = bool(module.path) or ('.' in module.name)
            is_extension_module = (module.code is None) and (module.file is not None)
            # Zip if requested, or if it's a single file standalone Python
            # module
            should_zip = ((not in_package) and not is_extension_module) \
                            or (pkgname in self.zipPackages)

            if is_extension_module:
                # Extension modules:
                if not should_zip:
                    # Copy the extension module in as a regular file.
                    # It may have already been copied above along with package
                    # data, but we need to do it this way in order to check
                    # for other libraries that it links against.
                    target = os.path.join(targetDir,
                                "/".join(module.name.split(".")[:-1]),
                ...
                # Save a Python loader in the zip file, and
            ...
            if should_zip:
                # Write the module to the zip file
                zinfo = zipfile.ZipInfo(fileName, zipTime)
                if self.compress:
                    zinfo.compress_type = zipfile.ZIP_DEFLATED
                outFile.writestr(zinfo, data)

            else:
                # Write the module to a regular file
                target = os.path.join(targetDir, fileName)
                self._CreateDirectory(os.path.dirname(target))
