...
set out_path=C:\TEMP\x64
rd %out_path% /s /q
call C:\python27\scripts\pyinstaller run.py --distpath=%out_path% --workpath=C:\TEMP\pyinst1 --name=%name% --onedir --icon=icon.ico --hidden-import=scipy.special._ufuncs_cxx --hidden-import=scipy.sparse.csgraph._validation --hidden-import=matplotlib.backends.backend_tkagg --noconsole 
