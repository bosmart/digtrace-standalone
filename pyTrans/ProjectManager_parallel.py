#project manager when opening files are in parallel. needs more work to account for different settings of separate files.
...
from Transformer import MyDropTarget
...
class ProjectManager():
        ...
        # parse the project structure
            ...
            try:
                with open(path, 'r') as fp:
                    ...
                    self.parent.Title = self.parent.Title + ' - ' + tail
    ...
    def open_transformer(self, doc, iszip, root):
        ...
        if 'bottom_panel' in doc:
            ...
            if self.compare_hash(model['hash'], model_path):
                    mpls = self.load_bottom_csv(paths, iszip, root, doc['bottom_panel'])
            for i in range(0,doc['bottom_panel'].__len__()):
                    model = doc['bottom_panel'][i]
                    if 'panel_master' in model:
                        print "load panel master"
                        self.load_transformer_prints(0, mpls[i])  # panel_master
                    if 'panel_source' in model:
                        print "load panel source"
                        self.load_transformer_prints(1, mpls[i])  # panel_source
    ...
    def load_bottom_csv(self, paths, iszip, root, models):

        model=models[0]

        result = self.run_job(paths, Loader(model['precision'], model['multiplier']))
        ...
        for xyzi, xyz, fname, guessed_multiplier in result:
            if xyzi is None:
                ...
                mpl = MatplotPanel(self.transformer.panel_thumbs, xyzi, xyz, models[iter]['multiplier'], models[iter]['precision'],
                                   title=os.path.basename(fname), fname=fname, lmark_xy=xy, A=np.asarray(models[iter]['A']))
        ...
        self.transformer.panel_thumbs.SendSizeEvent()
        return mpls
